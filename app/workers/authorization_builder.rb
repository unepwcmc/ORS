class AuthorizationBuilder
  ...
  def perform(user_id, questionnaire_id, url)
    begin
      ...
      authorization.save
      UserMailer.authorisation_granted(user, questionnaire, url).deliver
      logger.info("#{Time.now}: Built submission states of questionnaire: #{questionnaire.title}, for user #{user.full_name}. User has been notified through email to:  #{user.email}")
    rescue => e
